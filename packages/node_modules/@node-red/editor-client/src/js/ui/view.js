...
RED.view = (function() {
                ...
                try {
                    ...
                    try {
                        ...
                        if (hideLabel) {
                            ...
                            nn.h = Math.max(node_height,(nn.outputs || 0) * 15, (nn.inputs || 0) * 15);
                        } else {
                            ...
                            nn.h = Math.max(6+24*labelParts.lines.length,(nn.outputs || 0) * 15, 30, (nn.inputs || 0) * 15);
                        }
                    } catch(err) {
                    }
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode === RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                ...
                // make sure a line dragged from a node input, that has multiple inputs, is rendered properly
                // this patch could be written in a more verbose and understandable way but simply doing:
                /* if dragged line is PORT_TYPE_INPUT, then numOutputs is really numInputs */
                // works very well
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode == RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                if (portType != drag_lines[i].portType && mouseup_node !== drag_lines[i].node) {
                    ...
                    let src,dst,src_port,input_port;
                    let oldDst;
                    let oldSrc;
                    if (drag_line.portType === PORT_TYPE_OUTPUT) {
                        ...
                        dst = drag_line.node;
                        src_port = portIndex;
                        input_port = drag_line.port;
                        oldSrc = dst;
                        if (drag_line.link) {
                            oldDst = drag_line.link.source
                        }
                    }
                    var link = {source: src, sourcePort:src_port, target: dst, inputPort:input_port};
                    if (drag_line.virtualLink) {
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        if (hideLabel) {
                            ...
                            d.h = Math.max(6+24*labelParts.lines.length,(d.outputs || 0) * 15, 30, (d.inputs || 0) * 15);
                        }
                        this.__hideLabel__ = hideLabel;
                    }
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if ((!isLink || (showAllLinkPorts === -1 && !activeLinkNodes[d.id])) && d.inputs === 0 && !inputPorts.empty()) {
                            ...
                            for (let i = 0; i < d.inputs; i++ ) {
                                ...
                                inputGroupPorts[0][0].__data__ = this.__data__;
                                inputGroupPorts[0][0].__portType__ = PORT_TYPE_INPUT;
                                inputGroupPorts[0][0].__portIndex__ = i;
                                inputGroupPorts.on("mousedown",function(d){portMouseDown(d,PORT_TYPE_INPUT,i);})
                                    .on("touchstart",function(d){portMouseDown(d,PORT_TYPE_INPUT,i);d3.event.preventDefault();})
                                    .on("mouseup",function(d){portMouseUp(d,PORT_TYPE_INPUT,i);} )
                                    .on("touchend",function(d){portMouseUp(d,PORT_TYPE_INPUT,i);d3.event.preventDefault();} )
                                ...
                                RED.hooks.trigger("viewAddPort",{node:d,el: this, port: inputGroup[0][0], portType: "input", portIndex: i})
                            }
                        }
                        ...
                        thisNode.selectAll(".red-ui-flow-port-input").each(function(d,i) {
                            ...
                            port.attr("transform",function(d){return "translate(-5,"+((y+13*i)-5)+")";})
                        });
            ...
            link.each(function(d) {
                ...
                if (d.added || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    ...
                    var source_y = -((numOutputs-1)/2)*13 +13*sourcePort;
                    let numInputs = d.target.inputs || 1;
                    var targetPort = d.inputPort || 0;
                    let target_y = -((numInputs-1)/2)*13 +13*targetPort;                    d.x1 = d.source.x+(d.source.w/2||0);
                    d.y1 = d.source.y+source_y;
                    d.x2 = d.target.x-(d.target.w/2||0);
                    d.y2 = d.target.y+target_y;
     ...
     */
     function createNode(type, x, y, z) {
        ...
        nn.h = Math.max(RED.view.node_height, (nn.outputs || 0) * 15, (nn.inputs || 0) * 15);
        nn.resize = true;
