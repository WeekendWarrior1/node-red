...
RED.nodes = (function() {
    ...
    function convertNode(n, opts) {
        ...
        if (n._def.category != "config" || n.type === 'junction') {
            ...
            var inputWires = links.filter(function(d){return d.target === n;});
            for (var j=0;j<inputWires.length;j++) {
                var w = inputWires[j];
                if (w.target.type != "subflow") {
                    if (w.inputPort < node.inputWires.length) {
                        // need sourceId and sourcePort to recreate link from json object
                        node.inputWires[w.inputPort].push({sourceId: w.source.id, sourcePort: w.sourcePort});
                    }
                }
            }
                    ...
                    for (var w2=0;w2<wires.length;w2++) {
                        if (node_map.hasOwnProperty(wires[w2])) {
                            if (n.z === node_map[wires[w2]].z) {
                                ...
                                // find target nodes's input which wire needs to connect to and add it to link props
                                for (let i in node_map[wires[w2]].inputWires) {
                                    for (let inputWire of node_map[wires[w2]].inputWires[i]) {
                                        // if nodes were copied, they will have new IDs so we also check node_map with their prior IDs
                                        if ((inputWire.sourceId === n.id || node_map[inputWire.sourceId].id === n.id) && inputWire.sourcePort === w1) {
                                            link.inputPort = parseInt(i);
                                        }
                                    }
                                }
                                if (link.inputPort === undefined) { link.inputPort = 0; }
                                addLink(link);
                                new_links.push(link);
                            } else {
